                      ...
                      <table className="w-full text-left border-collapse min-w-[700px]">
                        <thead>
                          <tr className="border-b border-zinc-100 bg-zinc-50/50">
                            ...
                            {(() => {
                              const pDatas = pricingData.filter(pd => pd.modelIds?.includes(mainModelId || model.id) || pd.id === (mainModelId || model.id));
                              return pDatas.some(pData => pData.versions.some(v => v.cachePrice !== undefined)) ? (
